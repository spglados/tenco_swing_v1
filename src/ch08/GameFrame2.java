...
public class GameFrame2 extends JFrame{
	...
	int player1X = 150;
	int player1Y = 300;
	int player1SizeWidth = 50 ;
	int player1SizeHeight = 50 ;
	
	int enemyX = 250;
	int enemyY = 420;
	int enemySizeWidth = 50 ;
	int enemySizeHeight = 50 ;
	
	int differenceX =  player1X - enemyX;
	int differenceY =  player1Y - enemyY;
	...
	private class ImagePanel extends JPanel implements Runnable{
		...
		protected void paintComponent(Graphics g) {
			...
			g.drawImage(player1, player1X, player1Y, player1SizeWidth, player1SizeHeight, null);
			g.drawImage(enemy1, enemyX, enemyY, enemySizeWidth, enemySizeHeight, null);
	...
	private void addEventListener() {
		...
		this.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				// 1. 스페이스를 눌렀을 떄 적군을 멈출 수 있도록 코드 수정
					flag = false;
				}
